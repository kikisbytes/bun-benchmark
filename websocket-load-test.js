    ...
    const res = ws.connect(url, params, function (socket) {
        ...
        socket.on('open', function open() {
           ...
           socket.setInterval(function timeout() {
              socket.send(JSON.stringify(generateMessage('SAY', __VU, randomString(10))));
        ...
        socket.on('message', function (data) {
            ...
            if (data.event === 'CHAT_MSG') {
                console.log(`VU ${__VU} received: ${data.userId} says: ${data.message}`);
            } else {
                console.log(`VU ${__VU} received unhandled message: ${data.message}`);
            }
        });
